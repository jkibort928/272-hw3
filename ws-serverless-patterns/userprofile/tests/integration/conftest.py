...
USERPROFILE_STACK_NAME = os.getenv('USERPROFILE_STACK_NAME', None)
globalConfig = {}
...
def clear_dynamo_tables():
    ...
    for table_name, sort_key in (
        (globalConfig['AddressTable'], 'address_id'),
        (globalConfig['FavoriteTable'], 'restaurant_id'),
    ):
        db_response = ddb_client.scan(
            TableName=table_name,
            AttributesToGet=['user_id', sort_key]
        )
...
def global_config(request):
    ...
    globalConfig.update(get_stack_outputs(USERPROFILE_STACK_NAME))
